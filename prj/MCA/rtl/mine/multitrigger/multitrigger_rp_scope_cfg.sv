...
Variante multitrigger: registro de fuentes (máscara OR de 32 b/canal), UN
registro por canal en 0x240 + 4*GV (0x240=ch0, 0x244=ch1, ...), libre y
word-aligned, separado del SW-trigger y del trg_state de 0x04; config del
trigger_shield en 0x210. El resto del mapa es idéntico al rp_scope_cfg
original.
 */

module multitrigger_rp_scope_cfg #(
  parameter CHN  = 0 ,
  parameter N_CH = 2 ,
  parameter DW   = 14,
  parameter RSZ  = 14  // RAM size 2^RSZ
   ...
   // trigger_shield config (registro 0x210)
   ...
   input      [      17-1: 0] trig_snapshot_i        // sticky: qué disparó el último evento
...
// Registro de fuentes (máscara OR de 32 b): UN registro por canal en
// 0x240 + 4*GV (0x240=ch0, 0x244=ch1, ...). Direcciones libres y
...
if (GL == 0) begin
  ...
  assign trg_src_x[(GL+1)*32 -1:GL*32 ] = trg_src[(GL+1)*32-1:GL*32] ;
  ...
  assign trg_src_x[(GL+1)*32 -1:GL*32 ] = indep_mode[GL] ? trg_src[(GL+1)*32-1:GL*32]   : trg_src[31:0]    ;
...
if (adc_rstn_i == 1'b0) begin
  ...
  if (sys_wen) begin
    ...
    // multitrigger: configuración del trigger_shield (registro global)
    if (sys_addr[19:0]==20'h210) begin
      shield_src <= sys_wdata[ 3: 0] ;
      shield_dst <= sys_wdata[11: 8] ;
      shield_dur <= sys_wdata[31:16] ;
    end
    
    // SRC trigger space 210+32= 242
...
if (adc_rstn_i == 1'b0) begin
  ...
  casez (sys_addr[19:0])
    ...
    // multitrigger: lectura de la config del trigger_shield
    20'h00210 : begin sys_ack <= sys_en;          sys_rdata <= {shield_dur, 4'h0, shield_dst, 4'h0, shield_src} ; end
