...
rp_scope_multitrigger_com#(
  ...
  .mca_veto_o    (            ),  // sin MCA en este top
  ...
  // System bus del MULTITRIGGER — slot 7, 0x4070_0000
  .sys_mtrg_addr (sys[7].addr ),
  .sys_mtrg_wdata(sys[7].wdata),
  .sys_mtrg_wen  (sys[7].wen  ),
  .sys_mtrg_ren  (sys[7].ren  ),
  .sys_mtrg_rdata(sys[7].rdata),
  .sys_mtrg_err  (sys[7].err  ),
  .sys_mtrg_ack  (sys[7].ack  )
);
...
//
//  OJO: aca el MULTITRIGGER va al slot 7, no al 3.
//  Este top conserva el ASG (slot 2) y el PID (slot 3), asi que el slot 3
//  canonico no esta libre. El registro SLOTS (0x4060_0024) publica el mapa
//  real de ESTE bitstream: el software lo lee en vez de asumirlo.
////////////////////////////////////////////////////////////////////////////////
integration_cfg #(
  .MAGIC    (32'h494E5447),           // "INTG"
  .VERSION  (32'h0001_0000),
  .GITH     (GITH),
  .EN_OSC(1), .EN_MTRG(1), .EN_MCA(0), .EN_RING(0), .EN_ASG(1), .EN_PID(1),
  .SLOT_OSC(1), .SLOT_RING(2), .SLOT_MTRG(7), .SLOT_TOP(6), .SLOT_MCA(7),
  .N_CH(SCOPE_N_CH), .DW(SCOPE_DW), .RSZ(SCOPE_RSZ), .EN_FILT(1),
