...
module tb_event_ring_top;
  ...
  // La ruta llega por plusarg porque depende del CWD desde donde se lance vsim
  // (modulos/event_ring/work/). El Makefile inyecta la ruta absoluta al arbol
  // de software; el default deja el volcado al lado, para correr a mano.
  string ring_dump_path = "ring_dump.hex";
  ...
  task automatic dump_ring(input string fname);
    ...
    begin
      fd = $fopen(fname, "w");
      if (fd == 0) begin
        $display("  FAIL: no pude abrir '%s' para el volcado del ring", fname);
        fail_cnt = fail_cnt + 1;
        return;
      end
  ...
  initial begin
    ...
    void'($value$plusargs("ring_dump=%s", ring_dump_path));
    dump_ring(ring_dump_path);
