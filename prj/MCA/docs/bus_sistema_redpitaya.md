...
0x4010_0000  ├──────────────────────┤  slot 1   rp_scope_multitrigger_com
0x4020_0000  ├──────────────────────┤  slot 2   red_pitaya_asg
0x4030_0000  ├──────────────────────┤  slot 3   red_pitaya_pid
0x4040_0000  ├──────────────────────┤  slot 4   red_pitaya_ams     (XADC/PDM)
0x4050_0000  ├──────────────────────┤  slot 5   red_pitaya_daisy
0x4060_0000  ├──────────────────────┤  slot 6   sys_bus_stub       (libre*)
0x4070_0000  ├──────────────────────┤  slot 7   mca_top            (en el build del MCA)
0x4080_0000  └──────────────────────┘
```

\* El stub del slot 6 vive dentro de un `` `ifdef Z20_G2 `` en
`red_pitaya_top.sv:705-751`, así que **no está libre en todas las variantes**.
El del slot 7 es incondicional: es el que conviene ocupar.
...
**Estado actual: latente, no activo.** El driver mapea
`SCOPE_SIZE = 0x30000` (`multitrigger_utils.py:40`), o sea offsets
`0x00000`–`0x2FFFF`: nunca toca las aperturas rotas. Se dispararía con un
`N_CH` distinto, un barrido de direcciones, o un off-by-one en un lazo de
lectura.
