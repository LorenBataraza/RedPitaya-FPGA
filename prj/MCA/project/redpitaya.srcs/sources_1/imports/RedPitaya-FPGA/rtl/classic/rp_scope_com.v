...
// Es el módulo trigger para cada canal
rp_adc_trig #(
  .DW  (  DW     )
) i_adc_trig (
   // global signals
...
rp_trig_src #(
  ...
  .trig_dis_clr_i ( trig_dis_clr[GV] ),   // Trigger clear
...
rp_axi_sm #(
  ...
  .adc_rst_do_i     ( adc_rst_do[GV]                    ),
  .adc_we_keep_i    ( adc_we_keep[GV]                   ),
  .adc_arm_do_i     ( adc_arm_do[GV]                    ), // Salida del SM 
  .adc_trig_i       ( adc_trig[GV]                      ),
  .indep_mode_i     ( indep_mode[GV]                    ),
  ...
  .axi_wp_cur_o     ( axi_wp_cur[(GV+1)*32-1:GV*32]     ), // Curl?
