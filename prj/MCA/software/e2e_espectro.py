...
Uso:
    ...
    python3 e2e_espectro.py --acquire --duration 20

    # donde sea: graficar lo adquirido
    python3 e2e_espectro.py datos/e2e_espectro_<stamp>.npz
"""
import argparse
import glob
import os
...
def acquire(args):
    ...
    run_dir = os.path.join(HERE, 'datos', f'e2e_espectro_{stamp}')
    ...
    out = os.path.join(HERE, 'datos', f'e2e_espectro_{stamp}.npz')
    np.savez_compressed(
        out,
...
def main():
    ...
    if args.acquire:
        ...
        cands = sorted(glob.glob(os.path.join(HERE, 'datos', 'e2e_espectro_*.npz')))
        if not cands:
