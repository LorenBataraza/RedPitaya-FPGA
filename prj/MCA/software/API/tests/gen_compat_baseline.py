...
refactor). Produce dos ficheros que `test_compat_api.py` usa como referencia:
    ...
    API/tests/datos_ref/compat_baseline.json   superficie: nombres, tipos, firmas, y el VALOR
                                 de cada constante de los tres módulos públicos
    API/tests/datos_ref/compat_golden.npz      salidas numéricas de los helpers de análisis +
...
Uso:
    ...
    cd prj/MCA/software
    python API/tests/gen_compat_baseline.py           # escribe los dos ficheros
    python API/tests/gen_compat_baseline.py --check   # regenera y compara, sin escribir
...
BASELINE_JSON = os.path.join(_AQUI, 'datos_ref', 'compat_baseline.json')
GOLDEN_NPZ    = os.path.join(_AQUI, 'datos_ref', 'compat_golden.npz')
...
def privados_usados_fuera(nombre_mod):
    ...
    `monte-carlo/pileup.py` importa `_fwhm_pts` de rigol_dg4162, y tres tests
    ...
    OJO CON EL FILTRO. Excluye la IMPLEMENTACIÓN de `API/` —que un módulo use su
    propio privado no es "uso desde afuera"— pero NO sus tests, que sí cuentan.
    El filtro tiene que compararse contra el path de `API/` resuelto desde la
    raíz del árbol, no con un `os.sep + 'API' in raiz`: con este fichero viviendo
    bajo `API/tests/`, esa forma matchea TODO subdirectorio recorrido, el walk no
    devuelve nada, y `_fwhm_pts` / `_escala_para_fwhm` desaparecen del baseline
    sin que falle nada. Silencioso y difícil de ver: por eso está escrito así.
    """
    api_impl = os.path.join(_RAIZ, 'API')
    api_tests = os.path.join(_RAIZ, 'API', 'tests')

    encontrados = set()
    for raiz, _, ficheros in os.walk(_RAIZ):
        if '__pycache__' in raiz:
            continue
        if raiz.startswith(api_impl) and not raiz.startswith(api_tests):
            continue
        for f in ficheros:
...
def submodulos(nombre_mod):
    ...
    `from mca.storage import ...`, `campanas/run_poisson_loss.py`
...
def construir():
    ...
    baseline = {
        'nota': ('superficie PRE-refactor; la compara API/tests/test_compat_api.py. '
