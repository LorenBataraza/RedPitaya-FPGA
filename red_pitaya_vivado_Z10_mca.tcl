################################################################################
# red_pitaya_vivado_Z10_mca.tcl — build del Analizador Multicanal (MCA)
#
# COPIA de red_pitaya_vivado_Z10.tcl (commit 04aab7b) con CUATRO diferencias,
# todas de una linea. El original NO se toca: lo usan los 18 proyectos de prj/
# y es un archivo de upstream que sigue recibiendo cambios, asi que editarlo
...
#
# NO hace falta regenerar FSBL ni device tree: ambos salen de la configuracion
# del PS7 (ip/systemZ10.tcl), que es la MISMA que la de red_pitaya_top.
################################################################################
...
# 5) restricciones propias del MCA (multicycle sobre los registros de config).
#    Es la QUINTA diferencia con el original; ver el encabezado.
...
synth_design -top mca_red_pitaya_top -flatten_hierarchy none -bufg 16 -keep_equivalent_registers
